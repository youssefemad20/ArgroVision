...
window.cropProfiles = {
  wheat: {name:'Wheat', rainThreshold:8, humidityThreshold:60, desc:'Wheat prefers moderate moisture; avoid waterlogging.', growth:'80% (Good)', recommended_mm:8, projected_yield:'4.2 tons/ha', tips:'Keep soil evenly moist; fertilize at tillering and booting stages.', image:'paper.jpg'},
  ...
  apples: {name:'Apples', rainThreshold:7, humidityThreshold:60, desc:'Apples need balanced water; monitor during fruit set and enlargement.', growth:'82% (Good)', recommended_mm:10, projected_yield:'30 tons/ha', tips:'Thin excess fruit and monitor calcium levels to avoid blossom end rot.', image:'paper.jpg'},
...
async function initDashboard(){
  ...
  const humidityData = extract(window.weatherRows, humidityKey);
    ...
    if(tempCanvas){
      const chart = renderLineChart(tempCanvas.getContext('2d'), labels, [{label:'Temperature (Â°C)', data:tempData, borderColor:'#ff6b6b', backgroundColor:'rgba(255,107,107,0.08)', tension:0.2, fill:true}]);
      // set current temp display
      const tempCurrent = document.getElementById('temp-current');
      if(tempCurrent && tempData.length) tempCurrent.textContent = Math.round(tempData[tempData.length-1]) + 'Â°C';
    }
    if(rainCanvas){
